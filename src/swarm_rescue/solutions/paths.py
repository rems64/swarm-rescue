...
class Path:
    ...
    def __init__(self, lines, drone):
        lines = Line.reOrganize(lines)
    ...
    def append(self, point: Point):
        ...
        for i, line in enumerate(self.lines):
            ...
            if d < closestDistance and self.drone.canSee(p):
                ...
                closestIndex = i
        try:
            self.lines[closestIndex].append(point)
        except ValueError:
            self.lines.append(Line(point, closestPoint))
        except TypeError:
            try: 
                self.lines[-1].append(point)
            except ValueError:
                self.lines[-1].end = point
        self.lines = Line.reOrganize(self.lines)
